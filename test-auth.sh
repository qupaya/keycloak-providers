...
curl -i --request GET http://localhost:8080/auth/realms/master/remoteBlacklistRealm/remote-blacklist

echo -e "\n\nSENDING AUTHENTICATED REQUEST. THIS SHOULD SUCCESSFULLY CREATE COMPANY AND SUCCESS WITH 200: ";
curl -i --request GET http://localhost:8080/auth/realms/master/remoteBlacklistRealm/remote-blacklist --header "Authorization: Bearer $ACCESS_TOKEN";
